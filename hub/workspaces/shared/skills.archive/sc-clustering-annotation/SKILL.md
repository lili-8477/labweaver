---
name: sc-clustering-annotation
description: Leiden / Louvain clustering, resolution selection, manual marker-based cell type annotation, and automated annotation (CellTypist, scArches, Azimuth). Also covers dataset integration (scVI, scANVI, BBKNN, Harmony, scanorama). Use when assigning cell types or integrating multiple datasets.
---
...
# Transfer predictions to adata using the supported API
predictions.to_adata(insert_labels=True, insert_conf=True, inplace=True)
adata.obs["celltypist_cell_type"] = adata.obs["majority_voting"].astype(str)
